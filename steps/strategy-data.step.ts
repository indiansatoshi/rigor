...
export const handler = async (req: any, { state, logger }: any) => {
    logger.info('Fetching strategy data from state');

    // Initialize state if not already done
    const initialized = await state.get('app', 'initialized');
    if (!initialized) {
        logger.info('State not initialized, seeding with initial data');
        await state.set('app', 'goals', initialGoals);
        await state.set('app', 'initiatives', initialInitiatives);
        await state.set('app', 'opportunities', initialOpportunities);
        await state.set('app', 'epics', initialEpics);
        await state.set('app', 'leanCanvas', initialLeanCanvas);
        await state.set('app', 'metrics', initialMetrics);
        await state.set('app', 'initialized', true);
    }

    // Read from Motia state
    const companyGoals = await state.get('app', 'goals') || [];
    const leanCanvas = await state.get('app', 'leanCanvas') || [];
    ...
    const strategyMetrics = await state.get('app', 'metrics') || [];
