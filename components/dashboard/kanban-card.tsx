            ...
            <CardContent className="p-4 space-y-3">
                ...
                <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {normalizedPriority && (
                            <StatusBadge priority={normalizedPriority} className="h-5 px-1.5 text-[10px]" />
                        )}
                    </div>
                </div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                        {labels.map((label, index) => (
                            <Badge
                                key={index}
                                variant="secondary"
                                className="text-[10px] px-1.5 py-0 h-5 font-normal bg-secondary/50 text-secondary-foreground hover:bg-secondary"
                            >
                                {label}
                            </Badge>
                        ))}
                    ...
                    {allAssignees.length > 0 && (
                        <div className="flex items-center -space-x-2">
                            {allAssignees.map((user, index) => (
                                <Avatar key={index} className="h-6 w-6 border-2 border-background ring-1 ring-background transition-transform hover:z-10 hover:scale-110">
